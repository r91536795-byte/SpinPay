...
def main_menu(user_id):
    ...
    markup.add(
        types.InlineKeyboardButton("+ Пополнить", callback_data="menu_deposit"),
        types.InlineKeyboardButton("Вывести", callback_data="menu_withdraw")
    )
    markup.add(types.InlineKeyboardButton("👨‍💻 Поддержка", callback_data="menu_support"))
    if user_id in get_admins() or user_id == MAIN_ADMIN:
        markup.add(types.InlineKeyboardButton("⚙️ Admin", callback_data="menu_admin"))
    return markup
...
Служба поддержки: {SUPPORT}"""
    ...
    bot.send_message(msg.chat.id, f"{EMOJI['lightning']} Выберите действие:", parse_mode='HTML', reply_markup=main_menu(msg.from_user.id))
...
def menu_handler(call):
    ...
    if call.data == "menu_deposit":
        if not is_bot_active() and uid not in get_admins() and uid != MAIN_ADMIN:
            bot.send_message(call.message.chat.id, f"{EMOJI['off']} Бот на тех. обслуживании.", parse_mode='HTML')
            return

        temp_data[call.message.chat.id] = {"platform": "1xBet"}
        ...
        bot.send_message(call.message.chat.id,
            f"{EMOJI['support']} <b>Помощь:</b> {SUPPORT}",
            parse_mode='HTML')

    elif call.data == "menu_admin":
        if uid in get_admins() or uid == MAIN_ADMIN:
            bot.send_message(call.message.chat.id,
                f"{EMOJI['admin']} <b>Админ-панель {BOT_NAME}</b>",
...
def get_account_id(msg):
    ...
    bot.send_message(msg.chat.id,
        f"{EMOJI['money']} <b>Введите сумму (от 100 до 100 000 сом):</b>",
        parse_mode='HTML', reply_markup=back_menu())
