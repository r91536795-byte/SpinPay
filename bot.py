...
def send_media_bulletproof(chat_id, file_id, caption=None, reply_markup=None):
    ...
    try:
        ...
        try:
            ...
            if caption:
                bot.send_message(chat_id, caption + "\n\n⚠️ Не удалось загрузить изображение", parse_mode='HTML', reply_markup=reply_markup)
            return None
...
def get_amount(msg):
    ...
    qr = get_last_qr()
    if qr:
        send_media_bulletproof(user_id, qr, caption=f"{EMOJI['wallet']} <b>ОПЛАТИТЕ {amount:,.2f} сом</b>\n{EMOJI['clock']} 5 минут")
    else:
        ...
        for admin in get_admins():
            try:
                bot.send_message(admin,
                    f"{EMOJI['check']} Авто-одобрена заявка #{dep_id}\n👤 {user_id}\n💰 {amount:,.2f} сом\n🆔 {safe_html(account_id)}")
            except:
                pass
    else:
        # ===== РУЧНАЯ ПРОВЕРКА =====
